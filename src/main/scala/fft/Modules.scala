...
class IntgerComplex extends MyComplex with Config {
  val re = SInt((DataWidthIn).W)
  val im = SInt((DataWidthIn).W)
}
...
class ComplexAdd[T <: MyComplex](complex:T) extends Module with Config {
  ...
  if(use_float) {
    ...
    io.res.re := io.op1.re.asSInt + io.op2.re.asSInt 
    io.res.im := io.op1.im.asSInt + io.op2.im.asSInt 
...
object ComplexAdd extends Config{
  def apply(op1: MyComplex, op2: MyComplex): MyComplex = {
    val complex = if(use_float) new MyFloatComplex else new IntgerComplex
...
class ComplexMul[T <: MyComplex](complex:T) extends Module with Config {
  val io = IO(new ComplexIO(complex))
  io.res.re :=0.U.asTypeOf(complex.re)
  io.res.im :=0.U.asTypeOf(complex.im)
  if(use_float) {
    if(useGauss) {
    ...
    val a = io.op1.re.asSInt()
    val b = io.op1.im.asSInt()
    val c = io.op2.re.asSInt()
    val d = io.op2.im.asSInt()
    io.res.re :=(a*c-b*d)//.asTypeOf(complex.re)
    io.res.im :=(a*d+b*c)//.asTypeOf(complex.im)
    //if (useGauss) {
